  ...
  if (item && !item->GetProperty("pluginreplacecontextitems").asBoolean())
  {
    ...
    else
    {
      if (item->IsGame())
      {
        ...
        CGameManager::Get().GetGameClientIDs(*item, candidates);
...
bool CGUIWindowGames::OnContextButton(int itemNumber, CONTEXT_BUTTON button)
{
  ...
  case CONTEXT_BUTTON_PLAY_ITEM:
    if (item)
    {
      ...
      // If a zip file was selected, play the it as a file, not a directory
      if (url.GetProtocol() == "zip" && url.GetFileName() == "")
        g_application.PlayMedia(CFileItem(url.GetHostName(), false));
      else
