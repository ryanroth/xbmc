...
const CSavegameInfoTag& CSavegameInfoTag::operator=(const CSavegameInfoTag& right)
{
...
bool CSavegameInfoTag::GetDateTime(CDateTime &datetime) const
{
  return false;
}

bool CSavegameInfoTag::Load(const CStdString &path)
{
  m_isLoaded = false;
...
void CSavegameInfoTag::SetCamera(CStdString make, CStdString model)
{
  m_camera.clear();
  make.Trim();
  model.Trim();
...
void CSavegameInfoTag::SetTags(CStdString csvTags)
{
  m_tags.clear();
...
void CSavegameInfoTag::Archive(CArchive& ar)
{
  if (ar.IsStoring())
  {
    ar << m_isLoaded;
...
void CSavegameInfoTag::Serialize(bson *document) const
{
...
void CSavegameInfoTag::Deserialize(const bson *document, int dbId)
{
...
void CSavegameInfoTag::ToSortable(SortItem& sortable)
{
  
}

void CSavegameInfoTag::GetStringFromArchive(CArchive &ar, char *string, size_t length)
{
  CStdString temp;
  ar >> temp;
...
const CStdString CSavegameInfoTag::GetInfo(int info) const
{
  if (!m_isLoaded)
    return "";
...
int CSavegameInfoTag::TranslateString(const CStdString &info)
{
...
void CSavegameInfoTag::SetInfo(int info, const CStdString& value)
{
  switch (info)
  {
  case SLIDE_RESOLUTION:
...
void CSavegameInfoTag::SetLoaded(bool loaded)
{
  m_isLoaded = loaded;
}
